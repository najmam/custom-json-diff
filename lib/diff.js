...
function diff (from, to) {
  ...
  if (_.isEqual(from, to)) {
    return [{ op: "keep", path: [], newValue: from, oldValue: from }];
    ...
    ops = keys.map(function (key) {
      ...
      if (hasFrom && hasTo) {
        ...
        return { op: "remove", path: [key], oldValue: from[key] };
      } else {
        return { op: "add", path: [key], newValue: to[key] };
      }
    });
    ...
    if (matrix) {
      ...
      o.forEach(function (op) {
        if (op == "add") {
          ops.push({ op: "add", path: [j], newValue: to[j] });
          j++;
        } else if (op == "remove") {
          ops.push({ op: "remove", path: [j], oldValue: from[j] });
          i++;
        } else if (op == "replace") {
          ...
          ops.push({ op: "keep", path: [j], oldValue: from[j], newValue: from[j] });
          i++;
          j++;
        }
      });
    ...
    return [{ op: "replace", path: [], oldValue: from, newValue: to }];
  }
}
